...
function getUserGender() {
  return currentUser?.user_metadata?.gender || null;
}
function getCharacterImageUrl() {
  const custom = currentUser?.user_metadata?.character_image_url;
  if (custom) return custom;
  const gender = getUserGender();
  return gender === "female" ? "assets/default-female.png" : "assets/default-male.png";
}
async function saveGenderSelection(gender) {
  try {
    const { data, error } = await supabaseClient.auth.updateUser({ data: { gender } });
...
async function saveCharacterSelection(characterUrl) {
  try {
    const { data, error } = await supabaseClient.auth.updateUser({
      data: { character_image_url: characterUrl }
    });
    if (error) throw error;
    currentUser = data.user;
  } catch (err) {
...
function renderStarField(count, includePurple) {
  let stars = "";
  for (let i = 0; i < count; i++) {
    ...
    const isPurple = includePurple && i % 4 === 0;
    stars += `<div class="star${isPurple ? ' purple' : ''}" style="left:${left}%; top:${top}%; width:${size}px; height:${size}px; animation-duration:${duration}s; animation-delay:${delay}s;"></div>`;
  }
  return `<div class="star-field">${stars}</div>`;
}
...
function applyStoredTheme() {
  ...
  try {
    ...
    if (stored === "zenith" || stored === "dark") theme = stored;
  } catch (err) { /* localStorage unavailable — default to dark */ }
  document.body.setAttribute("data-theme", theme === "zenith" ? "zenith" : "dark");
}

function setTheme(theme) {
  document.body.setAttribute("data-theme", theme === "zenith" ? "zenith" : "dark");
  try { localStorage.setItem(THEME_KEY, theme); } catch (err) { /* non-fatal */ }
}

function getCurrentTheme() {
  return document.body.getAttribute("data-theme") === "zenith" ? "zenith" : "dark";
}
...
function getGreeting() {
  ...
  if (h >= 5 && h < 12) return "[SYSTEM] Good morning";
  if (h >= 12 && h < 17) return "[SYSTEM] Good afternoon";
  return "[SYSTEM] Good evening";
}
...
// ===== PROFILE SHEET (with theme switcher) =====
function openProfileSheet() {
  ...
  sheet.innerHTML = `
    ...
    <div class="bottom-sheet">
      ...
      <div class="account-portrait-wrap">
        <img class="account-portrait-img" src="${escapeAttr(getCharacterImageUrl())}" alt="" />
      </div>
      <div class="sheet-section" style="padding-top:0;">
        <button class="btn btn-secondary btn-small" style="margin:0 auto; display:block;" onclick="openCharacterSelect('account')">Change Character</button>
      </div>
      <div class="sheet-divider"></div>
      <div class="sheet-player">
        <div class="sheet-avatar">${initial}</div>
        <div class="sheet-player-info">
          <div class="sheet-player-name">Player: ${escapeHtml(playerName)}</div>
          <div class="sheet-player-email">${escapeHtml(currentUser?.email || '')}</div>
        </div>
      </div>
      <div class="sheet-divider"></div>
      <div class="sheet-section">
        <div class="sheet-label">Theme</div>
        <div class="theme-option ${current === 'dark' ? 'selected' : ''}" onclick="selectTheme('dark')">
          <span class="theme-radio">${current === 'dark' ? '◉' : '○'}</span>
          <span>Dark System</span>
        </div>
        <div class="theme-option ${current === 'zenith' ? 'selected' : ''}" onclick="selectTheme('zenith')">
          <span class="theme-radio">${current === 'zenith' ? '◉' : '○'}</span>
          <span>Zenith</span>
        </div>
      </div>
      <div class="sheet-divider"></div>
...
function selectTheme(theme) {
  setTheme(theme);
  openProfileSheet(); // toggles closed
  openProfileSheet(); // reopens refreshed with new selection
}
...
function openHabitMenu(habitId, event) {
  ...
  menu.innerHTML = `
    <div class="habit-menu-item danger" onclick="deleteHabit('${habitId}'); closeHabitMenu();">Delete quest</div>
  `;
...
function renderAuthScreen() {
  const useBg = getCurrentTheme() !== "zenith";
  const formInner = `
      <h1 style="margin-top:${useBg ? '0' : '40px'}; margin-bottom:4px;">Discipline</h1>
      <p style="color:var(--text-secondary); margin-bottom:24px;">${authMode === "login" ? "Log in to continue" : "Create your account"}</p>
  ...
  if (useBg) {
    render(`
      <div class="auth-bg-wrap">
        ...
        <div class="auth-bg-form-layer">${formInner}</div>
      </div>`);
  } else {
    render(`<div class="page-content">${formInner}</div>`);
  }
}
...
function renderForgotPasswordScreen(confirmationShown) {
  render(`
    <div class="page-content">
      ...
      <p style="color:var(--text-secondary); margin-bottom:16px;">Enter the email you signed up with</p>
...
async function initApp() {
  ...
  try {
    ...
    if (appState.habits.length === 0) {
      if (!getUserGender()) { renderGenderSelect(); return; }
      renderOnboardingStep1();
      return;
    }

    if (!getUserGender()) { renderGenderSelect(true); return; }
...
// ===== MISS HANDLING ("WHY PROMPT" → SYSTEM ALERT STYLE) =====
...
function renderMissModal() {
  render(`
    <div class="miss-modal">
      <div class="miss-alert-icon">⚠</div>
      <div class="miss-system-label">SYSTEM ALERT</div>
      <h1 class="miss-heading">Yesterday's quests were not completed.</h1>
      <p class="miss-subtext">What happened?</p>
      <div class="miss-divider"></div>
      <textarea id="missInput" rows="5" placeholder="Write your reason here..." oninput="updateMissButtonState()"></textarea>
...
// ===== CHARACTER SELECTION SCREEN (defaults only — upload comes in Part 2) =====
let characterSelectReturnTo = "home";

function openCharacterSelect(returnTo) {
  closeProfileSheet();
  characterSelectReturnTo = returnTo || "home";
  const current = getCharacterImageUrl();
  render(`
    <div class="page-content">
      <button class="back-btn" onclick="closeCharacterSelect()">← Back</button>
      <h1>Select Character</h1>
      <p style="color:var(--text-secondary); margin-bottom:16px;">Choose a default or upload your own image.</p>
      <div class="char-select-grid">
        <div class="char-select-option ${current === 'assets/default-male.png' ? 'selected' : ''}" onclick="chooseDefaultCharacter('assets/default-male.png')">
          <img class="char-select-thumb" src="assets/default-male.png" alt="" />
          <div class="char-select-name">Default Male</div>
        </div>
        <div class="char-select-option ${current === 'assets/default-female.png' ? 'selected' : ''}" onclick="chooseDefaultCharacter('assets/default-female.png')">
          <img class="char-select-thumb" src="assets/default-female.png" alt="" />
          <div class="char-select-name">Default Female</div>
        </div>
      </div>
      <div class="char-upload-row">
        <label class="char-upload-label" for="charUploadInput" id="charUploadLabel">+ Upload Your Own</label>
        <input type="file" id="charUploadInput" accept="image/*" onchange="handleCharacterUpload(event)" />
      </div>
      <div id="charUploadError" class="error-msg"></div>
    </div>`);
}
...
async function handleCharacterUpload(event) {
  ...
  if (file.size > 5 * 1024 * 1024) {
    $("#charUploadError").textContent = "Image must be under 5MB.";
    return;
  }

  const label = $("#charUploadLabel");
  label.textContent = "Uploading...";
  $("#charUploadError").textContent = "";

  try {
    const ext = file.name.split('.').pop().toLowerCase();
    const path = `${currentUser.id}/character.${ext}`;
    ...
    await saveCharacterSelection(publicUrl);
    closeCharacterSelect();
  } catch (err) {
    label.textContent = "+ Upload Your Own";
    $("#charUploadError").textContent = "Upload failed: " + err.message;
  }
}

async function chooseDefaultCharacter(url) {
  render(`<div class="loading">Saving...</div>`);
  await saveCharacterSelection(url);
  closeCharacterSelect();
}

function closeCharacterSelect() {
  if (characterSelectReturnTo === "account") {
    goToToday();
    openProfileSheet();
  } else {
    renderDashboardView();
...
// ===== GENDER SELECT (onboarding step 0, also used as one-time prompt for existing users) =====
function renderGenderSelect(existingUserMode) {
  render(`
    <div class="page-content">
      ${existingUserMode ? '' : '<p class="subtext" style="margin-top:40px;">Step 0 of 4</p>'}
      <h1 style="${existingUserMode ? 'margin-top:40px;' : ''}">Choose your character</h1>
      <p style="color:var(--text-secondary); margin-bottom:16px;">This sets your default character image. You can change it anytime later.</p>
      <div class="gender-option-list">
        <div class="option-item" onclick="selectGenderAndContinue('male', ${!!existingUserMode})">Male</div>
        <div class="option-item" onclick="selectGenderAndContinue('female', ${!!existingUserMode})">Female</div>
        <div class="option-item" onclick="selectGenderAndContinue('male', ${!!existingUserMode})">Prefer not to say</div>
      </div>
    </div>`);
}

async function selectGenderAndContinue(gender, existingUserMode) {
  render(`<div class="loading">Saving...</div>`);
  await saveGenderSelection(gender);
  if (existingUserMode) {
    await continueInitAfterMissCheck();
  } else {
    renderOnboardingStep1();
...
function renderOnboardingStep3() {
  const habitsHtml = appState.onboardingData.habitList.map((habit, idx) => `
    <div class="habit-card habit-pending">
  ...
  render(`
    <div class="page-content">
      ...
      <h1>Your quests</h1>
      <p style="color:var(--text-secondary); margin-bottom:16px;">Based on your answers. Add or remove as needed.</p>
      <div id="habitListContainer">${habitsHtml}</div>
      <div class="add-habit-row">
        <input type="text" id="newHabitInput" placeholder="New Quest" />
...
function goToStep4() {
  if (appState.onboardingData.habitList.length === 0) { alert("Please add at least one quest."); return; }
...
function renderCheckIn(editMode) {
  ...
  const yesterdaySection = (appState.isFirstSession && !isEditing)
    ? `<p style="color:var(--text-secondary); font-size:14px; margin: 14px 0;">Welcome! This is your first day — no yesterday to review yet.</p>`
  ...
  const charUrl = getCharacterImageUrl();
  renderWithNav(`
    <div class="checkin-header-banner">
      ${renderStarField(9, true)}
      <img class="checkin-header-img" src="${escapeAttr(charUrl)}" alt="" />
      <div class="checkin-header-fade"></div>
    </div>
    <h1>${isEditing ? "Edit Check-In" : "Check-In"}</h1>
    <p style="color:var(--text-secondary); margin-bottom:20px;">${humanDate}</p>
...
function renderDashboardView() {
  ...
  const habitsHtml = appState.habits.map(habit => {
    ...
    return `
      <div class="habit-card ${isDone ? 'habit-done' : 'habit-pending'}">
        <div class="habit-card-row">
  ...
  renderWithNav(`
    ...
    <div class="character-card" onclick="openCharacterSelect('home')">
      <img class="character-card-img" src="${escapeAttr(getCharacterImageUrl())}" alt="" />
      <div>
        <div class="character-card-label">Your Character</div>
        <div class="character-card-cta">Tap to change</div>
      </div>
    </div>
    ...
    <div class="score-card">
      <div class="score-card-top">
        ...
        <span class="score-card-status">${scoreVal > 0 ? '' : 'In progress'}</span>
      </div>
      <span class="score-card-num">${scoreVal}</span>
    ...
    <div class="completion-card">
      ...
      <div class="completion-label">Quest Completion</div>
    </div>

    <div class="section">
      <h2 class="section-label">Daily Quests</h2>
      <div id="habitListContainer">${habitsHtml}</div>
      <div class="add-habit-row" style="margin-top:8px;">
        <input type="text" id="newDashHabitInput" placeholder="New Quest" />
...
async function deleteHabit(habitId) {
  if (!confirm("Remove this quest? This cannot be undone.")) return;
  try {
    ...
    renderDashboardView();
  } catch (err) { alert("Error deleting quest: " + err.message); }
}
...
// ===== HISTORY (now "Mission Log" as page title) =====
...
function renderScoreChart() {
  if (appState.weeklyScores.length === 0) {
    return `<div class="score-chart-card">
      <h2 class="section-label">Score History</h2>
...
function renderHistoryCalendar() {
  ...
  renderWithNav(`
    ...
    <div class="calendar-legend">
      ...
      <div class="legend-item"><span class="legend-swatch" style="background:#2A2A2A"></span>Before you started</div>
    </div>
    ${renderScoreChart()}
  `, "history");
}
...
function renderDayDetail() {
  ...
  renderWithNav(`
    ...
    <h1>${heading}</h1>
    <div class="divider"></div>
    <div class="section"><h2 class="section-label">Daily Quests</h2>
      ${habitsHtml || '<p class="subtext">No quests existed this day.</p>'}
    </div>
    <div class="divider"></div>
